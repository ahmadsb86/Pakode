...
// app.use('/POJ/add-testcase', express.static('add-testcase'));
...
//   Question.create({name: req.body.name, statement: req.body.statement}).then(result => res.send({id: result.id}));
// })

// app.post('/api/post-tc', async (req,res)=>{ 
//   const r = req.body
//   TC.create({ QuestionId: r.qid, in: r.in, out: r.out }).then(result => console.log(chalk.blue(`Created new TC: ${result}`)))
// })
...
app.post('/api/judge', async (req, res)=>{
  ...
  for(let i=0; i<testCase.length;i++){
    response = await judgeit(req.body.code, req.body.lang, testCase[i]).then((data)=>{
